...
| 機能 | 説明 |
| ---- | ---- |
| データ統合 | Ledger DB + Azure DB を統合した View を提供 |
| 匿名化 | `is_name_private` 等に基づきデータを変換 |
| キャッシュ | 高頻度アクセスのパフォーマンス最適化 |
| 監査ログ | 全アクセスを記録 |
| リアルタイム通知 | Supabase Realtime で変更を受信・配信 |

### 6.4. 認証方式

| 通信 | 認証方式 |
| ---- | -------- |
| ユーザー → Ledger | JWT (Supabase Auth) |
| Ledger → Hub | API Key + IP 制限 |
| Hub → Azure DB | API Key + IP 制限 |
| Hub → Polimoney | API Key + IP 制限 |
