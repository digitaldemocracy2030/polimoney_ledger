...
| #   | タスク                       | 状態      | 詳細                                                     |
| --- | ---------------------------- | --------- | -------------------------------------------------------- |
| P-1 | ポリシー変更通知機能         | 🔲 未着手 | 「重要な変更がある場合は、本サービス上で通知します」(11節) |
| P-2 | 画像エクスポート（ZIP）      | 🔲 未着手 | 領収書画像も含めた完全なデータエクスポート               |
| P-3 | お問い合わせフォーム         | 🔲 未着手 | 権利行使の請求先（アクセス権/訂正権/削除権）             |
...
- プライバシーポリシーのバージョン管理
- ログイン時に「ポリシーが更新されました」バナー表示
- 必要に応じて再同意を求める

**実装案:**

```typescript
// DB: policy_versions テーブル
// UI: ダッシュボードにバナー表示
// ログイン時に最終確認バージョンと比較
```
...
| 選択肢                 | 工数 | 運用                     |
| ---------------------- | ---- | ------------------------ |
| A. GitHub Issue へ誘導 | 低   | OSS として透明性あり     |
| B. Google Form 埋め込み | 低   | 管理しやすい             |
| C. 専用フォーム実装    | 中   | Supabase に保存          |

**推奨:** A + B のハイブリッド（GitHub Issue + 非公開用 Google Form）
...
| #   | タスク             | 状態      | 詳細                             |
| --- | ------------------ | --------- | -------------------------------- |
| M-1 | Hub 同期: 自動同期 | ⏳ 進行中 | 仕訳承認時の自動同期             |
...
プライバシーポリシー 7節・12節で「お問い合わせフォームよりご連絡ください」と記載。
