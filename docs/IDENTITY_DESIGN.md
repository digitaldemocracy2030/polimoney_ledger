...
| 項目 | 決定 | 備考 |
|------|------|------|
| Azure DB | **PostgreSQL** | 識別子マスタ + Hub データ |
| API 認証 | **ハイブリッド** | ユーザー: JWT、サービス間: API Key + IP制限 |
| 連携方式 | **Hub + Realtime** | Supabase Realtime で変更通知 |

### 6.2. Polimoney Hub 技術スタック

| 項目 | 選択 | 理由 |
|------|------|------|
| ランタイム | **Deno** | Ledger と統一、TypeScript ネイティブ |
| フレームワーク | **Hono** | 軽量、Edge 最適化 |
| ホスティング | **Deno Deploy** | グローバルエッジ配信 |
| キャッシュ | Redis (Azure Cache) | 高頻度アクセスの最適化 |
| DB | Azure PostgreSQL | 識別子マスタと同居 |
...
┌─────────────────────────────────────────────────────────────┐
│ ユーザー認証 (Ledger)                                       │
│   ブラウザ → Fresh → Supabase Auth                         │
│   認証方式: JWT (OTP / Password)                            │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│ サービス間認証                                              │
│   Ledger ↔ Hub ↔ Azure DB ↔ Polimoney                      │
│   認証方式: API Key + IP 制限                               │
│                                                             │
│   リクエストヘッダ:                                         │
│   - X-API-Key: {サービス固有のキー}                         │
│   - X-Service-Name: polimoney-ledger                        │
└─────────────────────────────────────────────────────────────┘
```
...
| Hub のドメイン | hub.polimoney.jp 等 | 要検討 |

---

## 更新履歴

- Azure DB: PostgreSQL に決定
- API 認証: ハイブリッド方式に決定
- 連携方式: Hub + Realtime に決定
- Hub 技術スタック: Deno + Hono に決定
- 初版作成
