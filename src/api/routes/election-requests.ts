...
// GET /election-requests - 選挙リクエスト一覧
electionRequestsRouter.get("/", async (c) => {
  ...
  try {
    ...
    if (error) {
      ...
      return c.json({ error: "選挙リクエストの取得に失敗しました" }, 500);
    }

    return c.json({ data });
  } catch (error) {
    console.error("Error fetching election requests:", error);
    return c.json({ error: "サーバーエラーが発生しました" }, 500);
  }
});

// POST /election-requests - 選挙リクエスト作成
electionRequestsRouter.post("/", async (c) => {
  ...
  try {
    ...
    if (error) {
      ...
      return c.json({ error: "選挙リクエストの作成に失敗しました" }, 500);
    }

    return c.json({ data }, 201);
  } catch (error) {
    console.error("Error creating election request:", error);
    return c.json({ error: "サーバーエラーが発生しました" }, 500);
  }
});
